...
class CovidNERExtractor:
    ...
    def extract_covid_entities(self, tweet: str) -> List[Dict]:
        try:
            ...
            for match in model_predictions:
    ...
    def process_tweet(self, tweet: str) -> Dict:
        entities = self.extract_covid_entities(tweet)
...
def extract_keywords_from_file(file_path, model_name='dslim/bert-base-NER', output_dir=None,
    ...
    file_name = Path(file_path).stem.replace("policlaim_test", "policlaim_entities_test")
...
def main():
    ...
    for file_path in input_dir.glob(file_pattern):
        ...
        if 'train' in file_path.name:
            print(f"Skipping file {file_path.name} as it does not contain 'test'")
            continue
