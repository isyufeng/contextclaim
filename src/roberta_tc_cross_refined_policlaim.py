...
class TweetEvidenceDataset(Dataset):
    def __init__(self, tweets, evidences, labels, tokenizer, max_length=200):
        self.tweets = list(tweets) if not isinstance(tweets, list) else tweets
        self.evidences = list(evidences) if not isinstance(evidences, list) else evidences
...
# [FIX 1] Added macro-averaged metrics
...
def train_model(model, train_loader, val_loader, warmup_ratio, criterion, optimizer,
    ...
    for epoch in range(num_epochs):
        ...
        # [FIX 1 cont.] Log macro metrics in epoch_metrics
        epoch_metrics = {
            "current_epoch": epoch,
...
def parse_args():
    ...
    parser.add_argument('--learning_rate', type=float, default=1.3e-4)
    parser.add_argument('--batch_size', type=int, default=12)
    parser.add_argument('--num_epochs', type=int, default=3)
    parser.add_argument('--warmup_ratio', type=float, default=0.011)
    parser.add_argument('--dropout_rate', type=float, default=0.22)
...
def load_data(file_path):
    full_file_path = "data/evidence/" + file_path
    df = pd.read_csv(full_file_path, on_bad_lines='skip', dtype={"tweet_id": str})
...
def main():
    ...
    wandbLogger = WandbLogger(
        project="AFC_CD_Evidence_web4good",
    ...
    train_ids, train_tweets, train_evidences, train_labels = load_data(args.prefix + "_train.csv")
    train_ids, val_ids, train_tweets, val_tweets, train_evidences, val_evidences, train_labels, val_labels = train_test_split(
        train_ids, train_tweets, train_evidences, train_labels,
        test_size=0.1, random_state=42, stratify=train_labels
    )
    test_ids, test_tweets, test_evidences, test_labels = load_data(args.prefix + "_test.csv")
    ...
    try:
        ...
        # [FIX 1 cont.] CSV columns with macro metrics
        evaluation_file = "../evaluation_results/taslp_tc_roberta_policlaim.csv"
        columns = [
            'Experiment Name',
