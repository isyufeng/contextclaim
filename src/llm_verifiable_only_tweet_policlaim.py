...
def generate_prompt(sample):
    system_message = """Determine if the input text contains verifiable claims.
...
def process_evidence_field(df, batch_size=1000):
    ...
    # Check if optional columns exist and add them to processing if they do
    optional_columns = ['query', 'page_id']
...
def load_and_process_data(chunk_size=1000):
    """
    Load and process data in chunks to minimize memory usage
    Updated to use only train and test files, and split train into train/dev
    """
    ...
    # Load data files - only train and test
    data_files = {
        'train_full': "data/evidence/CT22_claim/policlaim_context_train.csv",
        'test': "data/evidence/CT22_claim/policlaim_context_test.csv"
    }
    ...
    # Remove the train_full to save memory
...
def process_all_data(model_id, chunk_size=1000):
    """
    ...
    with track_memory():
        datasets = load_and_process_data(chunk_size)
...
def setup_training(args, model_id):
    """Setup optimized training configuration with model-specific settings for TRL 0.21.0"""
...
def train_and_evaluate(experiment_name, model, trainer, test_data, i, tokenizer):
    ...
    with track_memory():
        ...
        for j in range(0, len(test_data), batch_size):
            ...
            with torch.no_grad():
                batch_predictions = []
                for k in range(5):  # 5 predictions per sample
                    batch = batch.apply(lambda sample: generate_response(sample, model, tokenizer, k), axis=1)
                    batch_predictions.append(batch['prediction' + str(k)].tolist())
        ...
        test_data_copy['prediction0'] = all_pred0
        test_data_copy['prediction1'] = all_pred1
        test_data_copy['prediction2'] = all_pred2
        test_data_copy['prediction3'] = all_pred3
        test_data_copy['prediction4'] = all_pred4
        test_data_copy['majority_prediction'] = majority_predictions

        # Save predictions to CSV
        output_filename = f"prediction_results/only_tweet/{experiment_name}_test_{i}.csv"
...
def calculate_and_save_metrics(metrics_list, args):
    try:
        ...
        metrics_summary = {
            'accuracy': (statistics.mean(accuracy_t), statistics.stdev(accuracy_t) if len(accuracy_t) > 1 else 0),
            'precision': (statistics.mean(precision_t), statistics.stdev(precision_t) if len(precision_t) > 1 else 0),
            'recall': (statistics.mean(recall_t), statistics.stdev(recall_t) if len(recall_t) > 1 else 0),
            'f1': (statistics.mean(f1_score_t), statistics.stdev(f1_score_t) if len(f1_score_t) > 1 else 0),
            'consistency': (
            statistics.mean(consistency_t), statistics.stdev(consistency_t) if len(consistency_t) > 1 else 0)
        }
        ...
        # Save results to CSV
        evaluation_file = "../evaluation_results/web4good_llm_t_evaluation.csv"
        columns = [
...
def main(args):
    ...
    datasets, tokenizer = process_all_data(args.model_id)
    ...
    for i in range(num_runs):
        ...
        trainer = SFTTrainer(
            ...
            # Remove tokenizer and max_seq_length
            # packing=False,
        ...
        # Train and evaluate
        metrics = train_and_evaluate(args.experiment_name, model, trainer, test_data, i, tokenizer)
    ...
    # Calculate and save final metrics for test dataset
