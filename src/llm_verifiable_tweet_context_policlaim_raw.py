...
# [FIX 1] Extract system message as module-level constant to avoid duplication.
...
def generate_prompt(sample):
    ...
    full_prompt = (
            "### Instruction:\n" + _SYSTEM_MESSAGE +
            # [FIX 2] Use "Input text" consistently (was "Input text" here
            # but "Input tweet" in generate_test_prompt).
            "\n\n### Input text:\n" + tweet_text +
...
def generate_test_prompt(sample):
    ...
    full_prompt = (
            "### Instruction:\n" + _SYSTEM_MESSAGE +
            # [FIX 2] Was "Input tweet" — now "Input text" to match training.
            "\n\n### Input text:\n" + tweet_text +
            "\n\n### Additional information:\n" + evidence +
            "\n\n### Response:"
    )
...
def generate_response(sample, model, tokenizer, j):
    ...
    # [FIX 3] Add torch.no_grad() inside the function for consistency.
    with torch.no_grad():
...
def load_and_process_data(prefix, chunk_size=1000):
    ...
    data_files = {
        'train_full': f"data/kw_entity_linking/linked_entities/{prefix}_train.json",
        'test': f"data/kw_entity_linking/linked_entities/{prefix}_test.json",
    }
...
# [FIX 4] Removed unused get_peft_config() function — its logic diverged
# from setup_training() (e.g. Mistral had o_proj in one but not the other,
# Llama target_modules were swapped). Keeping only setup_training().
...
def setup_training(args, model_id):
    ...
    # [FIX 6] Removed fan_in_fan_out=True (PEFT 0.14+ removed it).
    ...
    training_args = SFTConfig(
        ...
        # [FIX 8] Add gradient_checkpointing_kwargs for PEFT compatibility.
...
def calculate_and_save_metrics(metrics_list, args):
    try:
        ...
        # [FIX 9 cont.] CSV columns updated with macro metrics
        evaluation_file = "../evaluation_results/taslp_raw_tc_evaluation_PoliClaim.csv"
        columns = [
