...
# duplication and prevent potential inconsistencies between train/test prompts.
...
def generate_response(sample, model, tokenizer, j):
    ...
    # [FIX 2] Add torch.no_grad() inside the function itself for consistency
    # with the reference code, even though the caller may also wrap it.
    with torch.no_grad():
...
# ── JSON data loading (data-specific, kept as-is) ────────────────────────────

def _build_evidence_from_linked_entities(linked_entities: dict) -> str:
    """
    Build evidence string from linked_entities dict.
    Format: Entity_name-[extract]\nEntity_name-[extract]...
    """
    if not linked_entities or not isinstance(linked_entities, dict):
        return ""

    evidence_parts = []
    for entity_name, entity_data in linked_entities.items():
        if isinstance(entity_data, dict) and 'extract' in entity_data:
            extract_text = entity_data.get('extract', '')
            if extract_text:
                evidence_parts.append(f"{entity_name}-{extract_text}")
...
def process_json_to_dataframe(data):
    """
    Process JSON data into DataFrame, generating evidence field from linked_entities.
    """
    processed_data = []

    for idx, item in data.iterrows():
        try:
            linked_entities = item.get('linked_entities', {}) if isinstance(item.get('linked_entities'), dict) else {}
            evidence = _build_evidence_from_linked_entities(linked_entities)

            processed_item = {
                'tweet_id': str(item.get('tweet_id', f'missing_id_{idx}')),
                'tweet_text': str(item.get('tweet_text', '')),
                'class_label': int(item.get('class_label', 0)),
                'evidence': evidence,
                'linked_entities': linked_entities,
            }

            if not processed_item['tweet_text'].strip():
                print(f"Warning: Empty tweet text for item {idx}, skipping")
                continue

            processed_data.append(processed_item)

        except (ValueError, TypeError) as e:
            print(f"Warning: Error processing item {idx}: {e}, skipping")
            continue
...
def load_and_process_data(prefix, chunk_size=1000):
    ...
    # [FIX 3] Removed dev_test from data loading — evaluation on dev_test
    # is no longer performed.
    data_files = {
        'train': f"data/kw_entity_linking/linked_entities/{prefix}_train.json",
        'validation': f"data/kw_entity_linking/linked_entities/{prefix}_dev.json",
        'test': f"data/kw_entity_linking/linked_entities/{prefix}_test_gold.json",
    }
    ...
    for name, file_path in data_files.items():
        ...
        try:
            data = load_json_in_chunks(file_path, chunk_size=chunk_size)
            print(f"Initial {name} dataset size:", data.shape)

            processed_chunks = []
            for i in range(0, len(data), chunk_size):
                chunk = data.iloc[i:i + chunk_size].copy()
                chunk = process_json_to_dataframe(chunk)
                processed_chunks.append(chunk)
                if i % (chunk_size * 5) == 0:
                    clear_memory()

            data = pd.concat(processed_chunks, ignore_index=True) if processed_chunks else pd.DataFrame()
            datasets[name] = data
...
def prepare_datasets(datasets, tokenizer, chunk_size=1000):
    ...
    # [FIX 3 cont.] Only prepare test set (dev_test removed)
    for name in ['test']:
        print(f"Preparing {name} dataset...")
        test_chunks = []
        for i in range(0, len(datasets[name]), chunk_size):
...
def setup_training(args, model_id):
    # [FIX 4] Use bfloat16 instead of float16 throughout.
    # Transformers 5 + bnb 0.49: model config dtype is bfloat16 for modern
    # LLMs. fp16=True activates GradScaler which crashes on bf16 tensors.
    ...
    # [FIX 5] Removed fan_in_fan_out=True from all LoRA configs.
    # PEFT 0.14+ removed this parameter for non-Conv1D layers.
    ...
    # [FIX 6] Use SFTConfig instead of plain TrainingArguments.
    # SFTConfig (TRL 0.15+) is the preferred way to pass SFT-specific args.
    training_args = SFTConfig(
        ...
        # [FIX 7] Add gradient_checkpointing_kwargs for PEFT compatibility
        ...
        # [FIX 4 cont.] bf16=True + fp16=False
        bf16=True,
        fp16=False,
        weight_decay=0.001,
        max_grad_norm=1.0,
        ...
        # [FIX 6 cont.] SFTConfig-specific fields
...
# [FIX 3 cont.] Removed dev_test_data parameter — only evaluate on test set.
...
def calculate_and_save_metrics(metrics_list, args, dataset_name):
    try:
        ...
        # [FIX 9] Guard stdev with len>1 check to avoid crash on single run.
        def _safe_stdev(lst):
        ...
        evaluation_file = "../evaluation_results/taslp_raw_tc_evaluation_ct22.csv"
        columns = [
...
def main(args):
    ...
    for i in range(args.num_runs):
        ...
        model = AutoModelForCausalLM.from_pretrained(
            ...
            cache_dir='/gpfs/scratch/acw760/hf_cache',
            # [FIX 4 cont.] Use bfloat16 to match training config
            torch_dtype=torch.bfloat16,
        )
        model = optimize_model_memory(model)

        # [FIX 10] Use processing_class= instead of deprecated tokenizer=
        trainer = SFTTrainer(
            model=model,
